    ...
    it.each([
      {
        ...
        overrides: { NODE_ENV: NodeEnv.PRODUCTION },
        expected: { level: 'info', transport: undefined },
      },
      {
        name: 'uses LOG_LEVEL when set',
        overrides: { NODE_ENV: NodeEnv.PRODUCTION, LOG_LEVEL: 'debug' },
        expected: { level: 'debug', transport: undefined },
      },
    ])('$name', ({ overrides, expected }) => {
      expect(createLoggerOptions(makeEnv(overrides))).toEqual(expected);
    });
  });
});
