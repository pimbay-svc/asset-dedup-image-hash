...
describe('UDS server', () => {
  ...
  let hashBatch: ReturnType<typeof vi.fn>;
  let cradle: Cradle;

  beforeEach(async () => {
    dir = await mkdtemp(path.join(tmpdir(), 'uds-server-test-'));
    socketPath = path.join(dir, 'image-hash.sock');
    hashBatch = vi.fn().mockResolvedValue({ id1: { hash: 'deadbeef' } });
  ...
  it('completes a hash round trip over a single connection', async () => {
    ...
    expect(await responsePromise).toEqual({ outputs: { id1: { hash: 'deadbeef' } } });
    socket.destroy();
  });
  ...
  it('handles a second request on the same already-active connection', async () => {
    ...
    socket.write(encodeFrame({ op: 'hash', config: { algorithm: 'phash', hash_size: 8 }, inputs: {} }));
    await firstResponse;

    const secondResponse = nextFrame(socket, decoder);
    socket.write(encodeFrame({ op: 'hash', config: { algorithm: 'phash', hash_size: 8 }, inputs: {} }));

    expect(await secondResponse).toEqual({ outputs: { id1: { hash: 'deadbeef' } } });
    expect(hashBatch).toHaveBeenCalledTimes(2);
    socket.destroy();
  });
  ...
  it('rejects data from a second connection while one is already active', async () => {
    ...
    second.socket.write(encodeFrame({ op: 'hash', config: { algorithm: 'phash', hash_size: 8 }, inputs: {} }));
  ...
  it('an incomplete frame on a second connection does not claim the active slot', async () => {
    ...
    real.socket.write(encodeFrame({ op: 'hash', config: { algorithm: 'phash', hash_size: 8 }, inputs: {} }));
  ...
  it('closing a rejected (non-active) connection does not clear the real active connection', async () => {
    ...
    active.socket.write(encodeFrame({ op: 'hash', config: { algorithm: 'phash', hash_size: 8 }, inputs: {} }));
  ...
  it('survives a structurally malformed hash message instead of crashing the process', async () => {
    const { socket, decoder } = await connectFrames(socketPath);

    socket.write(encodeFrame({ op: 'hash', inputs: { id1: { path: '/shared/a.png' } } }));
    await new Promise((resolve) => setTimeout(resolve, 20));
